...
export const useListProduct = () => {
    ...
    const getInitialData = async () => {
        setIsLoading(true);
        const response = await service.product.getAll();
        setIsLoading(false);
      ...
      const handleDelete = async () => {
        ...
        const response = await service.product.remove(deleteItem?.id);
      ...
      const handleUpdate = (product) => {
        navigate(AppRouteName.product.edit, {
          state: { product, isUpdate: true },
        });
      };
